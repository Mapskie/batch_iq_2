...
export async function readFromExcel(filename: any): Promise<{ 
  ...
  const filePath = path.resolve('C:/Users/sean.michael.beredo/Downloads/sync-files/', filename)
  ...
  const batchWallTimeData = XLSX.utils.sheet_to_json(batchWallTimeSheet) as any[]
  ...
  return { readings: readings, workItems: workItems, wallTime: wallTime, batchLuw: batchLuw }
}

export async function GET(request: Request) {
  const { searchParams } = new URL(request.url);
  const filename = searchParams.get('file');
  const { readings, workItems, wallTime, batchLuw } = await readFromExcel(filename)

  return new Response(JSON.stringify({ readings, workItems, wallTime, batchLuw }), {
    status: 200,
