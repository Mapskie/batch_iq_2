...
type ReadingsProps = {
  itemData: any[]; // Ideally type this properly
};

export function ItemUploadBarChart({itemData}: ReadingsProps) {
  return (
      ...
      <CardContent className="h-[350px]"> {/* Set a fixed height for better layout control */}
        <ChartContainer config={chartConfig} className="h-full w-full">
          ...
          >
            ...
            {/* Bars */}
            <Bar dataKey="PrdData" fill="var(--color-PrdUploaded)" />
            <Bar dataKey="StgData" fill="var(--color-StgUploaded)" />
            <Bar dataKey="Variances" fill="var(--color-Difference)" />
          </BarChart>
        </ChartContainer>
      </CardContent>
    </Card>
